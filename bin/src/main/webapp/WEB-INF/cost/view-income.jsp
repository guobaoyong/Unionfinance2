...
<body onload="meg();">
	...
	<div id="body-one">
		<div class="table-responsive">
			<form action="${rootPath}cost/IncomeManager_Inquiry" method="post">
				<!-- 上面的提交表单 -->
				<div class="row">
					<!-- 输入下拉单一行 -->
					<div
		...
		<div class="container-two">
			...
			<div class="row">
				...
				<div
					...
					<nav>
						<ul class="pagination">
						<li><a href="${rootPath }cost/IncomeManager_${adss}?page=1">首页</a></li>
						<li><a
							href="${rootPath }cost/IncomeManager_${adss}?page=${allIncome.prePage}">上一页</a></li>
						<c:forEach var="i"
							...
							<c:choose>								
								<c:when test="${i>0 && i == allIncome.currentPage}">
									<li class="active"><a href="${rootPath }cost/IncomeManager_${adss}?page=${i }">${i}</a></li>
								</c:when>
								<c:when test="${i>0 && i != allIncome.currentPage}">
									<li><a href="${rootPath }cost/IncomeManager_${adss}?page=${i }">${i}</a></li>
								</c:when>
							</c:choose>
						</c:forEach>
						<li><a
							href="${rootPath }cost/IncomeManager_${adss}?page=${allIncome.nextPage}">下一页</a></li>
						<li><a
							href="${rootPath }cost/IncomeManager_${adss}?page=${allIncome.pageNum }">尾页</a></li>
					</ul>
					</nav>
				</div>
			</div>
