...
<body onload="meg();">
	...
	<div id="body-one">
		<div class="table-responsive">
			<form action="${rootPath}cost/CostManager_Inquiry" method="post">
				<!-- 上面的提交表单 -->
				<div class="row">
					<!-- 输入下拉单一行 -->
					<div
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<c:forEach items="${allCost.data}" var="pageCut" varStatus="status">
							<tr>	
								...
								<a href="${rootPath }cost/CostManager_toUpdateCost?co_id=${pageCut.co_id}&page=${allCost.currentPage}">修改</a>&nbsp;
								&nbsp; &nbsp;&nbsp;&nbsp;<img src="${rootPath }img/lajitong.png">
								<a href="${rootPath}cost/CostManager_deleteCost?cost.co_id=${pageCut.co_id}" style="color:#E11E05;" onclick="return del();">
								删除</a></td>
							</tr>
						</c:forEach>
				</tbody>
		...
		<div class="container-two">
			...
			<div class="row">
				...
				<div
					...
					<nav>
						<ul class="pagination">
					 	<li><a href="${rootPath }cost/CostManager_${adss}?page=1">首页</a></li>
					 	<li><a href="${rootPath }cost/CostManager_${adss}?page=${allCost.prePage}">上一页</a></li>
						<c:forEach var="i" begin="${allCost.currentPage-3>0?allCost.currentPage-3:1 }"
							...
							<c:choose>
								<c:when test="${i>0 && i == allCost.currentPage}">
									<li class="active"><a href="${rootPath }cost/CostManager_${adss}?page=${i }">${i}</a></li>
								</c:when>
								<c:when test="${i>0 && i != allCost.currentPage}">
									<li><a href="${rootPath }cost/CostManager_${adss}?page=${i }">${i}</a></li>
								</c:when>
							</c:choose>
						</c:forEach>
						<li><a href="${rootPath }cost/CostManager_${adss}?page=${allCost.nextPage}">下一页</a></li>
						<li><a href="${rootPath }cost/CostManager_${adss}?page=${allCost.pageNum }">尾页</a></li>
					 </ul>
					</nav>
				</div>
			</div>
