...
<body onload="meg();">
	...
	<div id="body-one">
		<div class="table-responsive">
			<form action="${rootPath}ordinary/ViewCost_Inquiry?union=${ordinary.company}" method="post">
				<!-- 上面的提交表单 -->
				<div class="row">
					<!-- 输入下拉单一行 -->
					<div
		...
		<div class="container-two">
			...
			<div class="row">
				...
				<div
					...
					<nav>
						<ul class="pagination">
					 	<li><a href="${rootPath }ordinary/ViewCost_${adss}?page=1&union=${ordinary.company }">首页</a></li>
					 	<li><a href="${rootPath }ordinary/ViewCost_${adss}?page=${allCost.prePage}&union=${ordinary.company }">上一页</a></li>
						<c:forEach var="i" begin="${allCost.currentPage-3>0?allCost.currentPage-3:1 }"
							...
							<c:choose>
								<c:when test="${i>0 && i == allCost.currentPage}">
									<li class="active"><a href="${rootPath }ordinary/ViewCost_${adss}?page=${i }&union=${ordinary.company }">${i}</a></li>
								</c:when>
								<c:when test="${i>0 && i != allCost.currentPage}">
									<li><a href="${rootPath }ordinary/ViewCost_${adss}?page=${i }&union=${ordinary.company }">${i}</a></li>
								</c:when>
							</c:choose>
						</c:forEach>
						<li><a href="${rootPath }ordinary/ViewCost_${adss}?page=${allCost.nextPage}&union=${ordinary.company }">下一页</a></li>
						<li><a href="${rootPath }ordinary/ViewCost_${adss}?page=${allCost.pageNum }&union=${ordinary.company }">尾页</a></li>
					 </ul>
					</nav>
				</div>
			</div>
